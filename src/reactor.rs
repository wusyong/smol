...
mod sys {
    ...
    #[cfg(target_os = "linux")]
    mod linux {
        pub use nix::libc::{itimerspec, timerfd_create, timerfd_settime, CLOCK_REALTIME, TFD_CLOEXEC};
        ...
        pub fn wait(&self, events: &mut Events, timeout: Option<Duration>) -> io::Result<usize> {
            let timeout_ms: isize = match timeout {
                ...
                Some(t) => unsafe {
                    ...
                    // Add timerfd to epoll and wait until timer fires.
                    let mut ev = EpollEvent::new(flags(), t.as_nanos() as u64);
                    epoll_ctl(self.0, EpollOp::EpollCtlAdd, *TFD, Some(&mut ev)).map_err(io_err)?;
                    epoll_wait(self.0, &mut [ev], -1).map_err(io_err)?;
                    epoll_ctl(self.0, EpollOp::EpollCtlDel, *TFD, None).map_err(io_err)?;
                    0
                },
